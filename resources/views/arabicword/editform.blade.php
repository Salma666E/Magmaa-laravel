...
<div class="page-wrapper bg-red p-t-180 p-b-100 font-robo">
      ...
      @if($errors->any())
        ...
        @endif
      <form action="{{url('/arabicword/editform'.$arabic_words->id)}} "method="post" >  
      {{csrf_field()}}
          <div class="input-group">
      <label for="id">رقم الاجتماع</label>
        <input type="text" class="input--style-2"  id="meeting_id" name="meeting_id"  value="{{$arabic_words->meeting_id}}">
</div>
 <div class="input-group">
        <label for="dep_id">الكلمه بالعربى </label>
        <input type="text" class="input--style-2" id="arabic_word" name="arabic_word"  value="{{$arabic_words->arabic_word}}">

</div>   
<div class="input-group">
        <label for="dep_id">المعنى</label>
        <input type="text" class="input--style-2" id="terms" name="terms"  value="{{$arabic_words->terms}}">
</div>
<div class="input-group">
        <label for="fname">وصف</label>
        <input type="text" class="input--style-2"id="definition" name="definition" value="{{$arabic_words->definition}}" >
</div>
<div class="input-group">
<label for="commission">commission</label>
        <input type="text" class="input--style-2"  id="commission" name="commission"  value="العربيه">
        <input class="btn btn-green"   style="background-color:black"type="submit" value="تعديل"> </inpute>
     </div>
      </form>
